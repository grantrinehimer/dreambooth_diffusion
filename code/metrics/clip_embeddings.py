...
def main():
    ...
    for file_name in sorted(os.listdir(args.real_dir)):
        file_path = os.path.join(args.real_dir, file_name)
        if os.path.isfile(file_path):
            real_emb.append(get_clip_img_embedding(clip, proc, file_path, device))

    gen_files = sorted(
        f for f in os.listdir(args.gen_dir)
        if os.path.isfile(os.path.join(args.gen_dir, f))
    )
    for file_name in gen_files:
        file_path = os.path.join(args.gen_dir, file_name)
